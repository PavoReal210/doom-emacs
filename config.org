...
This makes it so you don't have to manually load images each time. Images display automatically whenever an org buffer is opened (link-preview on by default).
  #+BEGIN_SRC emacs-lisp :tangle config.el
