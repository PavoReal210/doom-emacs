...
Programming Font: ~CozetteVector~
...
*** Org Mode Font Settings
    ...
    - What it all means:
      - Normally text is in the global =default= face so for general text we use =Overpass Nerd Font=.
      - For =fixed-pitch= mode which is sued by Org source blocks, Markdown blocks, and some UI elements, we want to specifically use =CozetteVector=.
      - =variable-pitch= is used for Org mode.
#+BEGIN_SRC emacs-lisp :tangle config.el
...
- Workflow: take a screenshot with =Super+Shift+S= (copies to clipboard), then use =SPC m c c= (=org-download-clipboard=) to paste it inline.
- Both =XDG_SESSION_TYPE= and =WAYLAND_DISPLAY= must be set — the daemon inherits neither from the compositor session.
#+BEGIN_SRC emacs-lisp :tangle config.el
...
Set as a float (0.0–1.0) so it adapts to any window width. 0.6 = 60% body text, 20% margin on each side — just enough centering to be comfortable without feeling like a narrow column.
#+BEGIN_SRC emacs-lisp :tangle config.el
...
- Paste image from clipboard: ~SPC m c c~
- Insert Xournal figure: ~SPC m c x~
- Display all inline images: ~SPC m d i~
- Undo: ~u~ (normal mode)
- Redo: ~C-r~ (normal and visual mode) — via =undo-fu-only-redo=
- Undo visualizer (vundo): ~C-x u~ — browse the full undo tree; navigate with =j=/=k=, every step restores the buffer to that state
...
#+END_SRC
*** Vundo
Visualizes the full undo/redo history as a tree in a dedicated buffer.
Walking the tree is a much easier way to redo multiple changes than repeating =C-r=.
Bound to ~C-x u~ (see the Hotkeys section). Navigate with =j=/=k=; every step restores the buffer to that state.
#+BEGIN_SRC emacs-lisp :tangle packages.el
(package! vundo)
#+END_SRC
*** Org-modern
...
#+end_src
** Completion
I use Company, Corfu, and Ivy for in-buffer completion. Vertico remains disabled to avoid clashing with Ivy.
#+begin_src emacs-lisp :tangle no
       :completion
